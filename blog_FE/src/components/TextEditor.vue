<template>
    <div>
      ...
      >
        ...
        <button type="button" @click="triggerFileInput" class="p-1">
          <ImageIcon title="Insert Image" />
        </button>
  
        <!-- Hidden file input -->
        <input type="file" ref="fileInputRef" @change="uploadImage" style="display: none;" accept="image/*" />
      </section>
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'text-white border border-gray-400 p-4 min-h-[12rem] max-h-[12rem] max-w-[52rem] overflow-y-auto outline-none prose max-w-none',
      },
    },
  })
